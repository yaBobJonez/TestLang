...
class Parser {
    private:
        ...
        Statement* assignmentState(){
        	...
        	} else{ std::cerr<<"Unsupported statement."; exit(EXIT_FAILURE); }
        }
        ...
        Expression* unary(){
            if(matches(TokenList::SUBTRACT)){
                Expression* intlValue = this->factor();
                return new UnaryNode('-', intlValue);
            }
            return this->factor();
        }
        Expression* factor(){
            ...
            } else{ std::cerr<<"Unrecognized token "<<std::to_string(curr.type)<<": "<<curr.value<<"."; exit(EXIT_FAILURE); }
        }
    public:
